import { hexToBigInt, keccak256, publicActions, stringToBytes } from 'viem';
...
import { toAddress } from '../types/index.js';
import { nowSeconds } from '../../utils/utils.js';
import { CrossChainVoucherCoordinator } from './CrossChainVoucherCoordinator.js';
import { _factorAmount, getSolventXlps, SolventXlpSorter } from '../../utils/getSolventXlps.js';
import { defaultFeeConfig, defaultXlpSelectionConfig } from '../config/index.js';
 ...
 */
const I_ENTRY_POINT_ID = computeInterfaceId(EntryPointMeta.abi);
/**
...
export class CrossChainBuilder {
    ...
    startBatch(chainId) {
        this.assertNotBuilt();
        const batch = new BatchBuilder(this, this.ephemeralSigner, this.coordinator, this.config, this.config.paymasters.addressOn(chainId), chainId);
    ...
    async buildVouchers() {
        ...
        await Promise.all(internalInfos.map(async (info) => {
    ...
    async collectXlpsPerVoucher() {
        await Promise.all(this.batchBuilders.flatMap(builder => builder.getOutVoucherRequests().map(async voucherReq => {
            await this.updateVoucherXlps(voucherReq, await this.getAllowedXlps(voucherReq));
        })));
    }
    async buildSingleChainBatches() {
        this.assertNotBuilt();
     ...
     */
    async getUserOpsToSign() {
        const batches = await this.buildSingleChainBatches();
        const userOps = batches.map(batch => batch.userOp);
        return userOps;
    }
    ...
    async initialize() {
        ...
        await Promise.all(this.config.chains.all().map(async (client) => {
            await this.checkChainConfiguration(client);
        }));
        this.initialized = true;
    }
    //check configuration for the given chain.
    async checkChainConfiguration(client) {
    ...
    async checkContract(name, client, contract, functionName, args, expectedRet) {
     ...
     */
    async _getVoucherSenderNonce(chainId) {
        let smartAccount = this.getAccount();
        return await this.config.paymasters.call(chainId, 'getSenderNonce', [smartAccount.addressOn(chainId)]);
    }
    /**
     ...
     */
    async getAllowedXlps(voucherRequestAction) {
        ...
        const { depositReserveFactor, includeBalance, minXlps, maxXlps, customXlpFilter } = policy;
        const tokensWithMinAmounts = tokens.map(t => ({
            token: t.token,
            amount: _factorAmount(depositReserveFactor, amountOrMinAmount(t))
        }));
        let xlps = await getSolventXlps(destChain, this.config.paymasters, tokensWithMinAmounts, includeBalance);
        const filtered = [];
    ...
    async updateVoucherXlps(voucherReq, xlps) {
    ...
    async dumpChainXlps(chainId, tokens) {
        console.log(`Dump of xlp liquidity on ${chainId}`);
        const ret = await getSolventXlps(chainId, this.config.paymasters, tokens.map(token => ({ token, amount: 0n })));
    ...
    getVoucherInternalInfo(voucher) {
    ...
    async sdkToVoucherRequest(voucherRequest) {
        ...
        return {
            origination: {
                ...
                assets: voucherRequest.tokens.map(asset => ({
                    ...
                    amount: asset.amount //amount can be runtimeVar.
                })),
            },
            destination: {
                chainId: destChainId,
                ...
                maxUserOpCost: 10000000000000000n, //todo: update after estimating userop gas
    ...
    async ensureEntryPointInterfaceSupport(chainId, entrypoints, client) {
...
function computeInterfaceId(abi) {
    let id = 0n;
    for (const item of abi) {
        ...
        const selector = hexToBigInt(('0x' + hash.slice(2, 10)));
        id ^= selector;
    }
    return id;
}
function toInterfaceHex(id) {
    return (`0x${id.toString(16).padStart(8, '0')}`);
}
//# sourceMappingURL=CrossChainBuilder.js.map