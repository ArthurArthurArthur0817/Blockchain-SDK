import { encodeDeployData, getContract, getCreate2Address, keccak256, padHex, publicActions, } from 'viem';
...
export async function deployCreateX(client) {
    const existingCode = await client.request({ method: 'eth_getCode', params: [createXaddress, 'latest'] });
    if (existingCode === '0x') {
        // console.log(`Using "hardhat_setCode" to inject CreateX on chain ${client.chain.id}`)
        await client.request({ method: 'hardhat_setCode', params: [createXaddress, CreateXMetadata.deployedBytecode] });
    }
}
...
export function getCreate2DeployAddress(metadata, ctrParams = [], salt = saltForBytecodeOnly) {
 ...
 * @param salt
 */
export async function create2deploy(client, metadata, ctrParams = [], salt = saltForBytecodeOnly) {
...
//# sourceMappingURL=create2deploy.js.map